...
class packet {
  public:
		...
		*/
		...
		volatile uint16_t air_speed;	// is the air speed relative to the payload in meter/second.
		volatile float software_state;			// the operating state of the software. (boot, idle, launch detect, deploy, etc.)
		volatile float particle_count;	// the decimalvalue telling measured particle countin mg/m3
		volatile int pitch;			// the tilt angle in the pitch axis in degrees. The resolution must be 1 degree.(EXTRA) 
		volatile int roll;			// the tilt angle of the roll axis in degrees. The resolution must be 1 degree.(EXTRA)
